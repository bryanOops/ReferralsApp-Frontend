...
export const TicketHandlers = [
  ...
  http.get('/api/data/refill/RefillData', () => {
    try {
      return HttpResponse.json({ status: 200, msg: 'Success', data: RefillData });
    } catch (error) {
  ...
  http.delete('/api/data/refill/delete', async ({ request }) => {
    try {
      const { id } = await request.json();
      const tickets = RefillData.map((ticket) =>
